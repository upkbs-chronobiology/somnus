...
class ExportSpec extends PlaySpec {
  ...
  "Export" should {
    "serialize answers to CSV" in {
      val csv = Export.asCsv(
        Seq("id", "questionId", "content", "userId", "timestamp", "timestampLocal"),
        Seq(
      ...
      csv must equal(""""id","questionId","content","userId","timestamp","timestampLocal"
    ...
    "properly serialize Options to CSV" in {
      val csv = Export.asCsv(Seq("Foo", "Bar", "Baz Booze"), Seq((Some("A"), Some(99), None)))
      csv must equal(""""Foo","Bar","Baz Booze"
                       |"A","99",
                       |""".stripMargin)
    }

    "properly serialize nulls to CSV" in {
      val csv = Export.asCsv(Seq(null, null, null, null), Seq((null, "test", null, "jest")))
      csv must equal(""",,,
                       |,"test",,"jest"
                       |""".stripMargin)
    }
    ...
    "throw in case of wrong headers shape" in {
      an[IllegalArgumentException] shouldBe thrownBy {
        Export.asCsv(Seq("a", "b"), Seq((1, 2, 3)))
      }
      an[IllegalArgumentException] shouldBe thrownBy {
        Export.asCsv(Seq("a", "b", "c"), Seq((1, 2)))
      }
      an[IllegalArgumentException] shouldBe thrownBy {
        Export.asCsv(Seq(), Seq((1, 2, 3)))
      }
    }
  }
}
