...
class StudyController @Inject() (
  ...
  def add = silhouette.SecuredAction(ForEditors).async { implicit request =>
    digestForm[StudyFormData](
      StudyForm.form,
      formData =>
        studyRepository
          .create(Study(0, formData.name))
  ...
  def update(id: Long) = silhouette.SecuredAction(ForEditors && acls.withStudyAccess(id, AccessLevel.Write)).async {
    implicit request =>
      digestForm[StudyFormData](
        StudyForm.form,
        formData =>
          studyRepository
            .update(Study(id, formData.name))
            .map(study => Ok(Json.toJson(study)))
      )
  }

  def delete(id: Long) = silhouette.SecuredAction(ForEditors && acls.withStudyAccess(id, AccessLevel.Write)).async {
    implicit request =>
      studyRepository
        .delete(id)
  ...
  def getParticipants(studyId: Long) =
    silhouette.SecuredAction(ForEditors && acls.withStudyAccess(studyId, AccessLevel.Read)).async { implicit request =>
      studyRepository
  ...
  def addParticipant(studyId: Long, userId: Long) =
    silhouette.SecuredAction(ForEditors && acls.withStudyAccess(studyId, AccessLevel.Write)).async { implicit request =>
      studyRepository
        .addParticipant(studyId, userId)
  ...
  def removeParticipant(studyId: Long, userId: Long) =
    silhouette.SecuredAction(ForEditors && acls.withStudyAccess(studyId, AccessLevel.Write)).async { implicit request =>
      studyRepository
        .removeParticipant(studyId, userId)
