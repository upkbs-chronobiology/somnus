...
class QuestionController @Inject() (
  ...
  def get(id: Long) = silhouette.SecuredAction(ForEditors && acls.withQuestionAccess(id, AccessLevel.Read)).async {
    implicit request =>
        ...
        formData =>
          checkAccess(
            ...
            formData.questionnaireId, {
              ...
              val answerRange =
                formData.answerRange.map(range => Serialization.serialize(InclusiveRange(range.min, range.max)))
              questionsRepo.add(
                Question(0, formData.content, formData.answerType, answerLabels, answerRange, formData.questionnaireId)
              ) map { newQuestion =>
  ...
  def update(id: Long) = silhouette.SecuredAction(ForEditors && acls.withQuestionAccess(id, AccessLevel.Write)).async {
    implicit request =>
      QuestionForm.form
        .bindFromRequest()
        .fold(
          ...
          formData => {
            ...
            questionsRepo
              .update(
                Question(id, formData.content, formData.answerType, answerLabels, answerRange, formData.questionnaireId)
              )
              .map { q =>
                Ok(Json.toJson(q))
              } recover {
  ...
  def delete(id: Long) = silhouette.SecuredAction(ForEditors && acls.withQuestionAccess(id, AccessLevel.Write)).async {
    implicit request =>
      questionsRepo
        .delete(id)
