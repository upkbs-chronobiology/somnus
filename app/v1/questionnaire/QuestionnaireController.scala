...
class QuestionnaireController @Inject() (
  ...
  def add() = silhouette.SecuredAction(ForEditors).async { implicit request =>
    digestForm[QuestionnaireFormData](
  ...
  def update(id: Long) =
    silhouette.SecuredAction(ForEditors && acls.withQuestionnaireAccess(id, AccessLevel.Write)).async {
      implicit request =>
        digestForm[QuestionnaireFormData](
          QuestionnaireForm.form,
          formData => {
      ...
      implicit request =>
        questionnaires
          ...
          .map {
            ...
            case 2 =>
              throw new IllegalStateException("Deleted more than one questionaire by id, but ids should be unique")
          }
          .recover {
  ...
  def duplicate(id: Long) =
    silhouette.SecuredAction(ForEditors && acls.withQuestionnaireAccess(id, AccessLevel.Read)).async {
      implicit request =>
        questionnaires
          .duplicate(id)
          .map(q => Created(Json.toJson(q)))
          .recover {
            case e: IllegalArgumentException => BadRequest(JsonError(e.getMessage))
          }
    }

  def getQuestions(questionnaireId: Long) =
    silhouette.SecuredAction(acls.withQuestionnaireAccess(questionnaireId, AccessLevel.Read)).async {
      implicit request =>
        questionsRepo
  ...
  private def permittedToWrite(user: User, formData: QuestionnaireFormData) = {
    formData.studyId
      .map(studyId => accessRules.mayAccessStudy(user, studyId, AccessLevel.Write))
      .toOptionFuture
      .map(_.getOrElse(accessRules.DisconnectedEntitiesPublic))
  }
}
