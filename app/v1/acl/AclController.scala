...
class AclController @Inject() (
  ...
  def listStudyAccessByUser(userId: Long) =
    silhouette.SecuredAction(acls.withStudyAccess(userId, AccessLevel.Read)).async { implicit request =>
      studyAccessRepo.listByUser(userId).map(seq => Ok(Json.toJson(seq)))
    }

  def listAccessByStudy(studyId: Long) =
    silhouette.SecuredAction(acls.withStudyAccess(studyId, AccessLevel.Read)).async { implicit request =>
  ...
  def put(userId: Long, studyId: Long) = studyOwnerAction(studyId).async { implicit request =>
    request.body.asJson.flatMap(json => json.apply(StudyAccess.LevelJsonKey).asOpt[AccessLevel]) match {
      ...
      case Some(level) =>
        studyAccessRepo
          .upsert(StudyAccess(userId, studyId, level))
